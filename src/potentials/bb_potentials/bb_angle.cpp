...
potential_shared_ptr new_bb_angle(){
	potential_shared_ptr ptr(new bb_angle());
	return ptr;
}

const double bb_angle::h = 1e-8;

bb_angle::bb_angle(){
	this->name_vector.clear();
	this->name_vector.push_back(potentials_name("bb_angle"));
...
double bb_angle::get_energy(const double ang, const double equilibrium_angle, const double half_angle_const) const{
	return (half_angle_const *
		pow(ang - equilibrium_angle, 2));
}

double bb_angle::get_energy(const PRODART::POSE::META::angle_element & ele) const{
...
double bb_angle::get_dE_dang(const double ang, const double equilibrium_angle, const double half_angle_const) const{
	return (half_angle_const *
		2.0 * (ang - equilibrium_angle));
}

double bb_angle::get_energy_ana_grad(const PRODART::POSE::META::angle_element& ele, UTILS::vector3d_vector& grad, const double weight ) const{
	if (ele.atom1_ptr->isActiveAndSet()
			&& ele.atom2_ptr->isActiveAndSet()
			&& ele.atom3_ptr->isActiveAndSet()){
...
double bb_angle::get_energy_grad(const PRODART::POSE::META::angle_element& ele, UTILS::vector3d_vector& grad, const double weight ) const{
	if (ele.atom1_ptr->isActiveAndSet()
			&& ele.atom2_ptr->isActiveAndSet()
			&& ele.atom3_ptr->isActiveAndSet()){
...
double bb_angle::get_energy(const PRODART::POSE::META::pose_meta_shared_ptr pose_meta_,
	...
	const angle_ele_vector& pair_list = bb_meta_dat->get_bb_angle_list();
...
double bb_angle::get_energy_with_gradient(const PRODART::POSE::META::pose_meta_shared_ptr pose_meta_,
		potentials_energies_map& energies_map) const{
	const bb_pose_meta_shared_ptr bb_meta_dat = static_pointer_cast<bb_pose_meta, pose_meta_interface>(pose_meta_);
...
void bb_angle::dummy_funct(double val) const{
	//double temp = val;
}
